...
export class ProduzioneComponent {
  ...
  tableParams: {
    ...
    globalSearch: string;
      ...
      totalRecords: 0,
      dateRange: [moment().subtract(1, 'week').toDate(), new Date()],
      dateRangeField: 'start_time',
      globalSearch: '',
    };
  ...
  async refreshData(
    ...
    let params: any = {
      order_by: this.tableParams.sortField,
      dir: this.tableParams.sortOrder,
      page: this.tableParams.page,
      size: this.tableParams.rows,
      range: this.tableParams.dateRangeField
    }

    if (this.tableParams.globalSearch) {
      params.globalfilter = this.tableParams.globalSearch;
    }

    if (this.tableParams.dateRange) {
      if (this.tableParams.dateRange[0]) {
        params['from_date'] = moment(this.tableParams.dateRange[0]).format('YYYY-MM-DD')
      }
      if (this.tableParams.dateRange[1]) {
        params['to_date'] = moment(this.tableParams.dateRange[1]).format('YYYY-MM-DD')
      }
    }

    this.httpSubscription = this.http
      .post(environment.API_URL + 'partslogs', params)
      .pipe(
      ...
      .subscribe((res: any) => {
        this.productionData = res?.content;
  ...
  clear() {
    ...
    this.tableParams.dateRangeField = 'start_time';
    this.refreshData();
  }
  ...
  customSFP(e: TableLazyLoadEvent) {
    ...
    if (e.sortField && !Array.isArray(e.sortField)) {
      ...
      this.tableParams.sortField = 'start_time';
    }
  ...
  */
  syncData() {
    if (!this.sse.isConnected) {
      this.synching = true;
      this.sse
        .connect(environment.API_URL + 'sse-parts-api', {}, [
          'progress',
          'complete',
          'error',
        ])
